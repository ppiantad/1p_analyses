    ...
    for ii = 1:size(folder_list, 1)
        ...
        sleap_folder_list = {};
            ...
            for sleap_folder = 1:size(sleap_folder_list, 1)
                ...
                idx = ~cellfun('isempty',strfind({files.name},'.avi')); %find the instances of .xlsx in the file list.
                %This command converts the name field into a cell array and searches
                %the cell array with strfind
                movie = files(idx); %build a mat file index
                clear idx files
                ...
                if isempty(movie)
                    ...
                    SLEAP_csv = strcat(sleap_folder_list{sleap_folder}, '\', movie.name);

                end
                % Check if all required files exist, if not, skip
                if exist('movie','var') && exist('ABET_file','var') && exist('GPIO_file','var')
