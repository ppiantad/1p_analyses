...
data_to_load = {'BLA_C_raw_no_additional_filtering_RDT_D1_only_completed_sessions_zall_window_base_workspace_11_categories_plus_start_for_PV_ITI.mat',...
    'BLA_C_raw_no_additional_filtering_Pre_RDT_RM_only_completed_sessions_zall_window_base_workspace_11_categories_plus_start_for_PV_ITI.mat',...
    'BLA_C_raw_no_additional_filtering_RDT_D1_only_completed_sessions_zall_window_base_workspace_12_categories_plus_start_risky_then_safe_for_PV_ITI.mat'};
...
if size(respClass_all_array, 2) == 11
    comparison_arrays_full = [11 11 11; 8 9 10]
    % comparison_arrays_full = [1 2 3; 5 6 7]

elseif size(respClass_all_array, 2) == 12
    comparison_arrays_full = [1 2 3 12; 8 9 10 11]
...
for aa = 1:size(comparison_arrays_full, 1)
    ...
    for gg = 1:size(animalIDs, 1)
        ...
        for ff = 1:size(caTraceTrials_mouse{select_mouse_index, comparison_arrays(1, 4)}, 2)
            for cc = 1:size(caTraceTrials_mouse{select_mouse_index,comparison_arrays(1, 4)}{1, ff}, 1)
                PV_ITI_all_mouse(cc, ff) = mean(caTraceTrials_mouse{select_mouse_index, comparison_arrays(1, 4)}{1, ff}(cc, ts1 >= -8 & ts1 <= 0));
        ...
        mean_PV_ITI_all_mouse = mean(PV_ITI_all_mouse)';
        ...
        for t = 1:size(ca_zscored, 2)
            ...
            similarityMatrix = corrcoef(activitySubset, mean_PV_ITI_all_mouse);
            ITI_similarityOverTime(t) = similarityMatrix(2);
            % end
        end
        % 
        ...
        %             PV_prechoice_all_mouse(cc, ff) = mean(caTraceTrials_mouse{select_mouse_index, comparison_arrays(1, 1)}{1, ff}(cc, ts1 >= -8 & ts1 <= 0));
        % 
        %         end
        % 
        %     end
        ...
        ITI_indices = ts1 >= ITI_window(1) & ts1 <= ITI_window(2);
        ...
        for t = 1:size(eTS,1)
            ...
            timeWin_ITI = [eTS(t, 3)+uv.evtWin(1,1):uv.dt:eTS(t, 3)+uv.evtWin(1,2)];  %calculate time window around each event
            ...
            if min(timeWin_prechoice_post_choice) > min(time_array) && max(timeWin_prechoice_post_choice) < max(time_array)    %if the beginning and end of the time window around the event occurred during the recording period. if not, the time window is out of range %if min(timeWin) > min(caTime) & max(timeWin) < max(caTime)
                ...
                idx_ITI = time_array > min(timeWin_ITI) & time_array < max(timeWin_ITI); 
                ...
                ITI_similarityOverTimeTrials(t,1:sum(idx_ITI)) = ITI_similarityOverTime(idx_ITI);
        ...
        ITI_period_b1_mean = mean(ITI_similarityOverTimeTrials(block_1_inds, ITI_indices), 2);
        ITI_period_b2_mean = mean(ITI_similarityOverTimeTrials(block_2_inds, ITI_indices), 2);
        ITI_period_b3_mean = mean(ITI_similarityOverTimeTrials(block_3_inds, ITI_indices), 2);
        ...
        ITI_over_time_mean(:, gg) = [ITI_period_b1_mean; ITI_period_b2_mean; ITI_period_b3_mean];
        ...
        ITI_period_b1_sem = std(ITI_similarityOverTimeTrials(block_1_inds, ITI_indices), 0, 2) / sqrt(length(find(block_1_inds == 1)));
        ITI_period_b2_sem = std(ITI_similarityOverTimeTrials(block_2_inds, ITI_indices), 0, 2) / sqrt(length(find(block_2_inds == 1)));
        ITI_period_b3_sem = std(ITI_similarityOverTimeTrials(block_3_inds, ITI_indices), 0, 2) / sqrt(length(find(block_3_inds == 1)));
        ...
        ITI_over_time_sem(:, gg) = [ITI_period_b1_sem; ITI_period_b2_sem; ITI_period_b3_sem];
        ...
        ITI_similarityOverTime_array{aa, gg} = ITI_similarityOverTime;
        ...
        ITI_similarityOverTimeTrials_array{aa, gg} = ITI_similarityOverTimeTrials;
    ...
    %%
    mean_ITI_over_time(:, aa) = mean(ITI_over_time_mean, 2);
    ...
    sem_ITI_over_time(:, aa) = mean(ITI_over_time_sem, 2);
    ...
    ITI_over_time_mean_iter{aa} = ITI_over_time_mean;
    ...
    ITI_over_time_sem_iter{aa} = ITI_over_time_sem;
...
shadedErrorBar(1:90, mean_ITI_over_time(:, 1), sem_ITI_over_time(:, 1), 'lineProps', {'color', 'r'});
hold on; shadedErrorBar(1:90, mean_ITI_over_time(:, 2), sem_ITI_over_time(:, 2), 'lineProps', {'color', 'k'});
...
%%
% Assuming postchoice_over_time_mean_iter is a 1x2 cell array, each containing a 90x10 double
data1 = ITI_over_time_mean_iter{1}; % First cell (90x10)
data2 = ITI_over_time_mean_iter{2}; % Second cell (90x10)
