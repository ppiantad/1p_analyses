...
if size(respClass_all_array, 2) == 10
    ...
    % comparison_arrays_full_full = [1 2 3; 5 6 7]
...
for kk = 1:size(animalIDs, 1)
    % prechoice_block_1 = respClass_all_array{1, comparison_arrays_full(1, 1)} == inhib_or_excite & respClass_all_array{1, comparison_arrays_full(1, 2)} ~= inhib_or_excite & respClass_all_array{1, comparison_arrays_full(1, 3)} ~= inhib_or_excite & respClass_all_array{1, 4} ~= inhib_or_excite;
    prechoice_block_1_mouse{kk, :} = respClass_all_array_mouse{kk, comparison_arrays_full(1, 1)} == inhib_or_excite & respClass_all_array_mouse{kk, comparison_arrays_full(1, 2)} ~= inhib_or_excite & respClass_all_array_mouse{kk, comparison_arrays_full(1, 3)} ~= inhib_or_excite;
    prechoice_blocks_2_and_3_mouse{kk, :} = respClass_all_array_mouse{kk, comparison_arrays_full(2, 1)} == inhib_or_excite & respClass_all_array_mouse{kk, comparison_arrays_full(2, 2)} ~= inhib_or_excite & respClass_all_array_mouse{kk, comparison_arrays_full(2, 3)} ~= inhib_or_excite;

    % postchoice_reward_block_1 = respClass_all_array{1, comparison_arrays_full(1, 2)} == inhib_or_excite & respClass_all_array{1, comparison_arrays_full(1, 1)} ~= inhib_or_excite & respClass_all_array{1, comparison_arrays_full(1, 3)} ~= inhib_or_excite & respClass_all_array{1, 4} ~= inhib_or_excite;
    postchoice_reward_block_1_mouse{kk, :}  = respClass_all_array_mouse{kk, comparison_arrays_full(1, 2)} == inhib_or_excite & respClass_all_array_mouse{kk, comparison_arrays_full(1, 1)} ~= inhib_or_excite & respClass_all_array_mouse{kk, comparison_arrays_full(1, 3)} ~= inhib_or_excite;
    postchoice_reward_blocks_2_and_3_mouse{kk, :}  = respClass_all_array_mouse{kk, comparison_arrays_full(2, 2)} == inhib_or_excite & respClass_all_array_mouse{kk, comparison_arrays_full(2, 1)} ~= inhib_or_excite & respClass_all_array_mouse{kk, comparison_arrays_full(2, 3)} ~= inhib_or_excite;

    % collect_block_1 = respClass_all_array{1, comparison_arrays_full(1, 3)} == inhib_or_excite & respClass_all_array{1, comparison_arrays_full(1, 1)} ~= inhib_or_excite & respClass_all_array{1, comparison_arrays_full(1, 2)} ~= inhib_or_excite & respClass_all_array{1, 4} ~= inhib_or_excite;
    collect_block_1_mouse{kk, :}  = respClass_all_array_mouse{kk, comparison_arrays_full(1, 3)} == inhib_or_excite & respClass_all_array_mouse{kk, comparison_arrays_full(1, 1)} ~= inhib_or_excite & respClass_all_array_mouse{kk, comparison_arrays_full(1, 2)} ~= inhib_or_excite;
    collect_blocks_2_and_3_mouse{kk, :}  = respClass_all_array_mouse{kk, comparison_arrays_full(2, 3)} == inhib_or_excite & respClass_all_array_mouse{kk, comparison_arrays_full(2, 1)} ~= inhib_or_excite & respClass_all_array_mouse{kk, comparison_arrays_full(2, 2)} ~= inhib_or_excite;

    num_cells_mouse(kk) = size(respClass_all_array_mouse{kk, 1}, 2);
    true_neutral_block_1_mouse{kk, :} = respClass_all_array_mouse{kk, comparison_arrays_full(1, 1)} == 3 & respClass_all_array_mouse{kk, comparison_arrays_full(1, 2)} == 3 & respClass_all_array_mouse{kk, comparison_arrays_full(1, 3)} == 3;
end
...
for aa = 1:size(comparison_arrays_full, 1)
    ...
    for gg = 1:size(animalIDs, 1)
        ...
        % if aa == 1
        %     pre_choice_cells_mouse = prechoice_block_1_mouse{gg, 1};
        %     post_choice_cells_mouse = postchoice_reward_block_1_mouse{gg, 1};
        %     consum_cells_mouse = collect_block_1_mouse{gg, 1};
        % 
        % elseif aa > 1
        %     pre_choice_cells_mouse = prechoice_blocks_2_and_3_mouse{gg, 1};
        %     post_choice_cells_mouse = postchoice_reward_blocks_2_and_3_mouse{gg, 1};
        %     consum_cells_mouse = collect_blocks_2_and_3_mouse{gg, 1};
        % 
        % end
        ...
        if aa == 1
            if sum_prechoice_block(gg, 1) >  sum_prechoice_block(gg, 2)
                pre_choice_neuron_list = find(prechoice_block_1_mouse{gg, 1} == 1);
                % Get the target size
                target_size = sum_prechoice_block(gg, 2);

                % Check if we need to downsample
                if length(pre_choice_neuron_list) > target_size
                    % Randomly sample without replacement
                    pre_choice_data = caTraceTrials_mouse{select_mouse_index, comparison_arrays(1, 1)};
                    neurons_to_keep = randsample(pre_choice_neuron_list, target_size);
                    neurons_to_discard = setdiff(pre_choice_neuron_list, neurons_to_keep);
                    pre_choice_data(:, neurons_to_discard) = [];
                end

                ca_zscored(neurons_to_discard, :) = [];

            else
                pre_choice_data = caTraceTrials_mouse{select_mouse_index, comparison_arrays(1, 1)};
            end

            for ff = 1:size(pre_choice_data, 2)
                for cc = 1:size(pre_choice_data{1, ff}, 1) % 10
                    PV_prechoice_all_mouse(cc, ff) = mean(pre_choice_data{1, ff}(cc, ts1 >= -4 & ts1 <= 0));
            ...
            for ff = 1:size(caTraceTrials_mouse{select_mouse_index, comparison_arrays(1, 1)}, 2)
                for cc = 1:size(caTraceTrials_mouse{select_mouse_index,comparison_arrays(1, 1)}{1, ff}, 1) % 10
        ...
        % PV_prechoice_all_mouse = PV_prechoice_all_mouse(:, pre_choice_cells_mouse == 1);
        ...
        % mean_PV_prechoice_all_mouse = mean_PV_prechoice_all_mouse(respClass_all_array_mouse{select_mouse_index, comparison_arrays_full(1, 1)} == 1, :);
        ...
        % y1_end = sum(respClass_all_array_mouse{select_mouse_index, comparison_arrays_full(1, 1)} == 1);  % End at the number of neurons in the first group
        % 
        ...
        % let's try zscoring the similarity over time to make them more
        % comparable across mice?
        % result: doesn't actually make a difference
        % prechoice_similarityOverTime = zscore(prechoice_similarityOverTime, [], 2);
        % 
        ...
        % % Define the position for the first green line
        % y1_start = 1;  % Start at the first neuron
        % y1_end = sum(respClass_all_array_mouse{select_mouse_index, comparison_arrays_full(1, 2)} == 1);  % End at the number of neurons in the first group
        % 
        % % Define the position for the second line
        % y2_start = y1_end + 1;  % Start right after the first group
        ...
        % % Define the position for the first green line
        % y1_start = 1;  % Start at the first neuron
        % y1_end = sum(respClass_all_array_mouse{select_mouse_index, comparison_arrays_full(1, 3)} == 1);  % End at the number of neurons in the first group
        % 
        % % Define the position for the second line
        % y2_start = y1_end + 1;  % Start right after the first group
