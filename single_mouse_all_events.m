...
% uv.evtWin = [-5 35; -10 10; -10 10]; %what time do you want to look at around each event [-10 10] [-5 35]
% uv.evtWin = [-10 10]; %what time do you want to look at around each event [-10 10] [-5 35]
% uv.BLper = [-5 0; -10 -5; -10 -5]; %what baseline period do you want for z-score [-10 -5] [-5 0]
% uv.BLper = [-10 -5]; %what baseline period do you want for z-score [-10 -5] [-5 0]
uv.dt = 0.1; %what is your frame rate (check neuron.Fs to be sure) 0.2
uv.behav = {'stTime','choiceTime','collectionTime'}; %which behavior/timestamp to look at

ABET_file_name = 'BLA-Insc-1 01202021.csv';
animalID = strsplit(ABET_file_name, ' ');
animalID = strrep(animalID{1}, '-', '_');


[BehavData,ABETfile,Descriptives, block_end]=ABET2TableFn_Chamber_A_v5(ABET_file_name,[]);
...
load('BLA-Insc-1_2021-01-20_RDT_D1.CNMF_final.mat');
...
%%
for zz = 1:length(uv.behav)
    alignment_event = char(uv.behav(zz));
    if strcmp(alignment_event,'stTime')
        uv.evtWin = [-5 35];
        uv.BLper = [-5 0];
    elseif strcmp(alignment_event,'choiceTime')
        uv.evtWin = [-10 10];
        uv.BLper = [-10 -5];
    elseif strcmp(alignment_event,'collectionTime')
        uv.evtWin = [-10 10];
        uv.BLper = [-10 -5];
    end
    for i = 1 %could loop through multiple mice like this if you had it
        ...
        for qq = 1:size(varargin_list, 1)
            [BehavData_filtered, trials, varargin] = TrialFilter(BehavData, (varargin_list{qq, 1}{1,:}));

            varargin_strings = string(varargin);
            ...
            eTS = BehavData_filtered.(alignment_event); %get time stamps

            if ~isempty(BehavData_filtered)
            ...
            for u = 1:size(ca,1)
                ...
                for t = 1:size(eTS,1)
                    ...
                    if min(timeWin) > min(frames3) & max(timeWin) < max(frames3)    %if the beginning and end of the time window around the event occurred during the recording period. if not, the time window is out of range %if min(timeWin) > min(caTime) & max(timeWin) < max(caTime)
                        ...
                        %                         caTraceTrials(t,1:numMeasurements) = unitTrace(idx);
                        zb(t,:) = mean(unitTrace(bl_idx)); %baseline mean
                        zsd(t,:) = std(unitTrace(bl_idx)); %baseline std
                        zb_window(t,:) = mean(unitTrace(idx)); %baseline mean
                        zsd_window(t,:) = std(unitTrace(idx)); %baseline std
                        tmp = 0;
                        for j = 1:size(caTraceTrials,2)
                            tmp = tmp+1;
                            zall_baseline(t,tmp) = (caTraceTrials(t,j) - zb(t))/zsd(t);
                ...
                unitXTrials(u).zall_baseline = zall_baseline;
            ...
            intermediate.(animalID).(alignment_event).(filter_args).time = frames3;
            intermediate.(animalID).(alignment_event).(filter_args).unitAVG = unitAVG;
            intermediate.(animalID).(alignment_event).(filter_args).unitXTrials = unitXTrials;
            intermediate.(animalID).(alignment_event).(filter_args).uv = uv;
            intermediate.(animalID).(alignment_event).(filter_args).unitSEM = unitSEM;
            intermediate.(animalID).(alignment_event).(filter_args).uv.BehavData = BehavData;
            
            elseif isempty(BehavData_filtered)
                intermediate.(animalID).(alignment_event).(filter_args) = [];
                disp('No trials of this type exist in this session for this mouse')
            end
            clear caEvtCtTrials caTraceTrials caEvtRateTrials unitTrace idx zall_window zall_baseline zb zsd zb_window zb_baseline unitTS unitTrace unitXTrials unitAVG unitSEM i
        end
%         clear unitTS unitTrace unitXTrials unitAVG unitSEM i 
...
%     %     final(i).day = i;
%     final.(animalID).(alignment_event).(i).time = frames3; %final(i).time = caTime;
