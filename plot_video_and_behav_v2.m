...
set(gca, 'Color', 'black', 'XColor', 'w', 'YColor', 'w');
xlabel('Time (seconds relative to current)', 'Color', 'w');
set(gca, 'YTick', []);
ax1 = gca;
...
try
    while current_frame <= max_frames && mpg_frame_count <= mpg_num_frames
        ...
        time_range = ((window_start:window_end) - current_frame) / calcium_frame_rate;  % Relative to current frame in SECONDS
        ...
        for k = 1:length(matching_start_frames)
            ...
            x_pos = (frame_idx - current_frame) / calcium_frame_rate;  % Convert to seconds
        ...
        % Set axis limits
        xlim([-window_size/calcium_frame_rate, window_size/calcium_frame_rate]);  % Convert to seconds
        ylim([0, 1]);
