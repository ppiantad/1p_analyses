%% SVM Context Decoding — D3 Session (Experimental vs One Context)
%
% D3 session structure (12 min total, 10 Hz):
%   Neutral  : frames   1 – 1200   (0–2 min)
%   Aversive : frames 1201 – 2400  (2–4 min)
%   Neutral  : frames 2401 – 3600  (4–6 min)
%   Aversive : frames 3601 – 4800  (6–8 min)
%   Neutral  : frames 4801 – 6000  (8–10 min)
%   Aversive : frames 6001 – 7200  (10–12 min)
...
neutral_blocks  = [1, 1200; 2401, 3600; 4801, 6000];
aversive_blocks = [1201, 2400; 3601, 4800; 6001, 7200];
...
for gg = 1:numel(target_groups)
    ...
    for mm = 1:numel(exp_mice)
        ...
        if n_timepoints < aversive_blocks(end, 2)
            fprintf('  WARNING: Only %d frames; expected >= %d. Skipping.\n', ...
                    n_timepoints, aversive_blocks(end, 2));
            continue
        end
...
for pp = 1:size(pairs, 1)
    ...
    else, bstr = sprintf('ns', p_posthoc(pp)); end
...
sgtitle('SVM Context Decoding — D3', 'FontWeight', 'bold');

out_fig = 'I:\MATLAB\my_repo\context fear\svm_context_decoding_D3.png';
