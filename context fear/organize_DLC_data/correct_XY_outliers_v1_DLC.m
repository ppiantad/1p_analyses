function [X_data_corrected, Y_data_corrected] = correct_XY_outliers_v1_GoPro(X_data, Y_data, first_frame)
...
%trim off the initial part of the video where the camera is being placed
%because this screws up the correction
X_data = X_data(first_frame:end, :);
Y_data = Y_data(first_frame:end, :);


motion_data = final_DLC.B46849.D3.DLC_data_raw.motion;
...
X_data_corrected = [zeros(round(first_frame)-1, size(X_data, 2)); X_data];
Y_data_corrected = [zeros(round(first_frame)-1, size(Y_data, 2)); Y_data];
