...
data_to_load = {'BLA_C_raw_no_additional_filtering_RDT_D1_only_completed_sessions_zall_window_base_workspace_10_categories.mat',...
    'BLA_C_raw_no_additional_filtering_Pre_RDT_RM_only_completed_sessions_zall_window_base_workspace_10_categories.mat',...
    'BLA_RM_D1_6_categories_03212025_including_spike_prob_array_counts.mat',...
...
elseif size(respClass_all_array, 2) == 3
    comparison_arrays_full = [1, 2, 3; 1, 2, 3]
end
...
for aa = 1:size(comparison_arrays_full, 1)
    ...
    for gg = 1:size(animalIDs, 1)
        ...
        if strcmp(select_mouse, 'BLA_Insc_25') & strcmp(first_session, 'RM_D1')
...
%% if you run all of the above for RDT, Late RM, and Early RM, saving off the mean_prechoice_over_time array and sem_prechoice_over_time 
% and editing the file to end with _RDT, _Late_RM, or _Early_RM


load('pre_choice_over_time_saved.mat')

prechoice_RDT_diff = mean_prechoice_over_time_RDT(:,1) - mean_prechoice_over_time_RDT(:, 2);
prechoice_Late_RM_diff = mean_prechoice_over_time_Late_RM(:,1) - mean_prechoice_over_time_Late_RM(:, 2);
prechoice_Early_RM_diff = mean_prechoice_over_time_Early_RM(:,1) - mean_prechoice_over_time_Early_RM(:, 2);

figure; plot(prechoice_RDT_diff)
hold on; plot(prechoice_Late_RM_diff)
hold on; plot(prechoice_Early_RM_diff)


% Extract the two SEM columns
sem1 = sem_prechoice_over_time_RDT(:, 1);
sem2 = sem_prechoice_over_time_RDT(:, 2);

% Propagated SEM
sem_difference = sqrt(sem1.^2 + sem2.^2);
...
%%
pre_choice_RDT_diff_mice = prechoice_over_time_mean_iter_RDT{1, 1}  - prechoice_over_time_mean_iter_RDT{1, 2};
pre_choice_RDT_sem_difference = sqrt(prechoice_over_time_sem_iter_RDT{1, 1}  .^2 + prechoice_over_time_sem_iter_RDT{1, 2}  .^2);

pre_choice_Late_RM_diff_mice = prechoice_over_time_mean_iter_Late_RM{1, 1}  - prechoice_over_time_mean_iter_Late_RM{1, 2};
pre_choice_Late_RM_sem_difference = sqrt(prechoice_over_time_sem_iter_Late_RM{1, 1}  .^2 + prechoice_over_time_sem_iter_Late_RM{1, 2}  .^2);

% pre_choice_Early_RM_diff_mice = prechoice_over_time_mean_iter_Early_RM{1, 1}  - prechoice_over_time_mean_iter_Early_RM{1, 2};
% pre_choice_Early_RM_sem_difference = sqrt(prechoice_over_time_sem_iter_Early_RM{1, 1}  .^2 + prechoice_over_time_sem_iter_Early_RM{1, 2}  .^2);

mean_data_array = {pre_choice_RDT_diff_mice', pre_choice_Late_RM_diff_mice'}
sem_data_array = {pre_choice_RDT_sem_difference', pre_choice_Late_RM_sem_difference'}
% [comparison, perm_p_sig] = perm_and_bCI_fn_analysis_PhilDBressel_for_1p(mean_data_array, sem_data_array, ts1, [-5 5], [-0.5 0.8], 3)
[comparison, perm_p_sig] = perm_and_bCI_fn_analysis_PhilDBressel_for_1p(mean_data_array, sem_data_array, [1:1:90], [1 90], [-0.3 0.3], 2)

mean_data_array = {pre_choice_RDT_diff_mice', pre_choice_Early_RM_diff_mice'}
sem_data_array = {pre_choice_RDT_sem_difference', pre_choice_Early_RM_sem_difference'}
% [comparison, perm_p_sig] = perm_and_bCI_fn_analysis_PhilDBressel_for_1p(mean_data_array, sem_data_array, ts1, [-5 5], [-0.5 0.8], 3)
[comparison, perm_p_sig] = perm_and_bCI_fn_analysis_PhilDBressel_for_1p(mean_data_array, sem_data_array, [1:1:90], [1 90], [-0.3 0.3], 2)

pre_choice_RDT_diff_mice_mean_safe = mean(pre_choice_RDT_diff_mice(1:30,:), 1)

% pre_choice_Early_RM_diff_mice_mean_safe = mean(pre_choice_Early_RM_diff_mice(1:30,:), 1)

pre_choice_Late_RM_diff_mice_mean_safe = mean(pre_choice_Late_RM_diff_mice(1:30,:), 1)

[h p ci stats_block1] = ttest2(pre_choice_RDT_diff_mice_mean_safe, pre_choice_Late_RM_diff_mice_mean_safe)


pre_choice_RDT_diff_mice_mean_risky = mean(pre_choice_RDT_diff_mice(31:90,:), 1)

% pre_choice_Early_RM_diff_mice_mean_risky = mean(pre_choice_Early_RM_diff_mice(31:90,:), 1)

pre_choice_Late_RM_diff_mice_mean_risky = mean(pre_choice_Late_RM_diff_mice(31:90,:), 1)

[h p ci stats_block23] = ttest2(pre_choice_RDT_diff_mice_mean_risky, pre_choice_Late_RM_diff_mice_mean_risky)
...
shadedErrorBar(1:90, mean(pre_choice_RDT_diff_mice, 2), mean(pre_choice_RDT_sem_difference, 2), 'lineProps', {'color', 'r'});
hold on; shadedErrorBar(1:90, mean(pre_choice_Late_RM_diff_mice, 2), mean(pre_choice_Late_RM_sem_difference, 2), 'lineProps', {'color', 'k'});
% hold on;shadedErrorBar(ts1, nanmean(neuron_mean_array{1, 8}(conserved_consumption==1, :)), nanmean(neuron_sem_array{1, 8}(conserved_consumption==1, :)), 'lineProps', {'color', 'b'});
xlim([1 90]);
% Set X-axis ticks
set(gca, 'XTick', [1, 30, 60, 90]);
...
%%
% Assuming postchoice_over_time_mean_iter is a 1x2 cell array, each containing a 90x10 double
data1 = pre_choice_Late_RM_diff_mice; % First cell (90x10)
data2 = pre_choice_RDT_diff_mice; % Second cell (90x10)
