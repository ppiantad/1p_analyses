...
%                     zall_to_BL_array(iter, neuron_num) = {final.(currentanimal).(session_to_analyze).(epoc_to_align).unitXTrials(qq).zall(trials,:)};
                ...
                neuron_mean_mouse_unnormalized{ii, iter+1}(qq,: ) = mean(caTraceTrials, 1);
                neuron_sem_mouse_unnormalized{ii, iter+1}(qq,: ) = nanstd(caTraceTrials,1)/(sqrt(size(caTraceTrials, 1)));
